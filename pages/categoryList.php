                ...
                <?php foreach ($jsonContent as $item) : ?>
                    <div class="card product-card">
                        <img
                                ...
                                alt="product image"/>
                        <br>
                        <h2><a href="<?= sprintf("/itemId.php?itemId=%s&categoryId=%s", $item["pid"], $categoryId) ?>"><?= $item["name"] ?></a></h2>
                        <h4 class="price-field"><?= $item["price"] ?></h4>
                        <a href="<?= sprintf("/itemId.php?itemId=%s&categoryId=%s", $item["pid"], $categoryId) ?>" class="btn btn-sm">details.</a>
                    </div>
                <?php endforeach; ?>
            <?php else : ?>
